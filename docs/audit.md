...
4. **Resolved 2026-09-24 — legacy `test/` removed.** Ben approved pruning all old copies; the v3 preview stays recoverable from Git history (last present at `a8ce109`).
...
No tracked or untracked file was deleted or moved. Town and space screenshots retain their labeled v9 graphics references. V10 adds the timer, end-of-run summary and statistics tests on top of the graphics exit contract, with docs distinguishing source evidence from deployment evidence and the prior v8 build. The historical `test/` snapshot remained intact at v10; it was removed on 2026-09-24 at Ben's request and is recoverable from Git history.
